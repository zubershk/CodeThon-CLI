...
const API_VARS = ['NVIDIA_API_KEY', 'OPENAI_API_KEY', 'ANTHROPIC_API_KEY', 'GROQ_API_KEY', 'DEEPSEEK_API_KEY', 'TOGETHER_API_KEY'];

const PROVIDER_SETUP: Record<string, { envVar: string; url: string; free: boolean }> = {
...
export function formatApiError(error: unknown, providerName?: string): string {
  ...
  if (isQuota) {
    ...
    lines.push(`  Or switch to a different model/providers with: ct model`);
  } else {
  ...
  if (fallbacks.length > 0) {
    lines.push(``, `  Or use a free provider:`);
    for (const [pname, info] of fallbacks) {
      lines.push(`    \u2022 ${getDisplayName(pname)}  \u2192  ${info.envVar}=<your-key>`);
